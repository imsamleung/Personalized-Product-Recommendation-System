...
CBF_item_similarity_matrix = pickle.load(open(
    f'{root_dir}/models/final/content_based_filtering/item_similarity_matrix.pkl', 'rb'))
...
def CBF_find_similar_items(product_id, df_products):
    ...
    sorted_scores = pd.Series(
        CBF_item_similarity_matrix[idx]).sort_values(ascending=False)
...
item_based_CF_knn_model = pickle.load(open(
    f'{root_dir}/models/final/item_based_collaborative_filtering/knn.pkl', 'rb'))
...
def item_based_CF_find_similar_items(product_id, df_products):
    ...
    df_1 = pd.DataFrame({'product_id': [item_based_CF_knn_model.trainset.to_raw_iid(
        inner_id) for inner_id in neighbors]})
...
def item_based_collaborative_filtering(user_id, product_id, n=10):
    df_similar_items = item_based_CF_find_similar_items(
        product_id, df_products)
    df_user_unrated_items = get_user_unrated_items(
        user_id, df_similar_items, df_reviews)
...
sentiment_analysis_model = pickle.load(open(
    f'{root_dir}/models/final/sentiment_analysis/logistic_regression_with_tfidf_vectorizer.pkl', 'rb'))


def predict_mean_sentiment(product_id: str):
    review_list = df_reviews[df_reviews['product_id']
                             == product_id]['processed_review_text'].tolist()
...
def recommend(user_id, product_id, n=10):
    ...
    df_item_based_CF_items = item_based_collaborative_filtering(
    ...
    df_item_based_CF_items['sen'] = scaler.transform(
        df_item_based_CF_items[['sen']])

    # Calculate the ranking score for each product
    w1 = 1
    w2 = 2
    df_item_based_CF_items['ranking_score'] = w1 * \
        df_item_based_CF_items['pre'] + w2 * df_item_based_CF_items['sen']
    ...
    if(com > 0):
        df_CBF_items = content_based_filtering(
            user_id, product_id, len(df_products))
        df_CBF_items = df_CBF_items[~df_CBF_items['product_id'].isin(
            df_item_based_CF_items['product_id'])]
